...
find ./paht/to/dir -type f -iname "*.mp4" -print0 | while IFS= read -r -d $'\0' file;
  do
    ...
    ##    use: 
    ##        2> /dev/null -> to show NO ffmpeg output
    ##        < /dev/null -> show the ffmpeg output
    ####
    #ffmpeg -i "tmp.mp4" -map_metadata -1 -c:v copy -c:a copy "tmp_no_subs.mp4" 2> /dev/null ## remove media tags
    ffmpeg -i "tmp.mp4" -sn -c copy "tmp_no_subs.mp4" 2> /dev/null ## remove subtitles
